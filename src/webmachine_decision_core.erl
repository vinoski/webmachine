...
respond({304, _ReasonPhrase}=CodeAndPhrase, Resource, EndTime) ->
    wrcall({remove_resp_header, ?CONTENT_TYPE}),
...
%% "Content-MD5 present?"
decision(v3b9) ->
    decision_test(get_header_val(?CONTENT_MD5), undefined, v3b9b, v3b9a);
%% "Content-MD5 valid?"
decision(v3b9a) ->
    case resource_call(validate_content_checksum) of
        ...
        {halt, Code} ->
            respond(Code);
        not_validated ->
            Checksum = base64:decode(get_header_val(?CONTENT_MD5)),
...
decision(v3b8) ->
    case resource_call(is_authorized) of
        ...
        {halt, Code}  ->
            respond(Code);
        AuthHead ->
            wrcall({set_resp_header, ?WWW_AUTHENTICATE, AuthHead}),
            respond(401)
    end;
%% "Forbidden?"
decision(v3b7) ->
...
decision(v3d4) ->
    decision_test(get_header_val(?ACCEPT_LANGUAGE),
                  undefined, v3e5, v3d5);
...
decision(v3e5) ->
    case get_header_val(?ACCEPT_CHARSET) of
...
decision(v3e6) ->
    decision_test(choose_charset(get_header_val(?ACCEPT_CHARSET)),
                  none, 406, v3f6);
...
decision(v3f6) ->
    ...
    wrcall({set_resp_header, ?CONTENT_TYPE, CType ++ CSet}),
    case get_header_val(?ACCEPT_ENCODING) of
        undefined ->
...
decision(v3f7) ->
    decision_test(choose_encoding(get_header_val(?ACCEPT_ENCODING)),
                  none, 406, v3g7);
%% "Resource exists?"
decision(v3g7) ->
    ...
    case variances() of
        [] -> nop;
        [Variance|_]=Variances when is_list(Variance) ->
            wrcall({set_resp_header, ?VARY, string:join(Variances, ", ")});
        [Variance|Variances] ->
            VaryBin = list_to_binary([Variance | [[", ", V] || V <- Variances]]),
            wrcall({set_resp_header, ?VARY, VaryBin})
    end,
    decision_test(resource_call(resource_exists), true, v3g8, v3h7);
%% "If-Match exists?"
decision(v3g8) ->
    decision_test(get_header_val(?IF_MATCH), undefined, v3h10, v3g9);
%% "If-Match: * exists"
decision(v3g9) ->
    decision_test(get_header_val(?IF_MATCH), "*", v3h10, v3g11);
%% "ETag in If-Match"
decision(v3g11) ->
    ETags = webmachine_util:split_quoted_strings(get_header_val(?IF_MATCH)),
...
decision(v3h7) ->
    decision_test(get_header_val(?IF_MATCH), undefined, v3i7, 412);
%% "If-unmodified-since exists?"
decision(v3h10) ->
    decision_test(get_header_val(?IF_UNMODIFIED_SINCE),undefined,v3i12,v3h11);
%% "I-UM-S is valid date?"
decision(v3h11) ->
    IUMSDate = get_header_val(?IF_UNMODIFIED_SINCE),
    decision_test(webmachine_util:convert_request_date(IUMSDate),
                  bad_date, v3i12, v3h12);
%% "Last-Modified > I-UM-S?"
decision(v3h12) ->
    ReqDate = get_header_val(?IF_UNMODIFIED_SINCE),
...
decision(v3i12) ->
    decision_test(get_header_val(?IF_NONE_MATCH), undefined, v3l13, v3i13);
%% "If-None-Match: * exists?"
decision(v3i13) ->
    decision_test(get_header_val(?IF_NONE_MATCH), "*", v3j18, v3k13);
%% GET or HEAD?
decision(v3j18) ->
...
decision(v3k13) ->
    ETags = webmachine_util:split_quoted_strings(get_header_val(?IF_NONE_MATCH)),
    decision_test_fn(resource_call(generate_etag),
...
%% "IMS exists?"
decision(v3l13) ->
    decision_test(get_header_val(?IF_MODIFIED_SINCE), undefined, v3m16, v3l14);
%% "IMS is valid date?"
decision(v3l14) ->
    IMSDate = get_header_val(?IF_MODIFIED_SINCE),
    decision_test(webmachine_util:convert_request_date(IMSDate),
                  bad_date, v3m16, v3l15);
%% "IMS > Now?"
decision(v3l15) ->
    NowDateTime = calendar:universal_time(),
    ReqDate = get_header_val(?IF_MODIFIED_SINCE),
    ReqErlDate = webmachine_util:convert_request_date(ReqDate),
    ...
    FinalBody = case BuildBody of
        true ->
            ...
            case resource_call(last_modified) of
                undefined -> nop;
                LM ->
                    wrcall({set_resp_header, ?LAST_MODIFIED,
...
accept_helper() ->
    accept_helper(get_header_val(?CONTENT_TYPE)).
        ...
        ChosenEnc ->
            case ChosenEnc of
                ?IDENTITY ->
                    nop;
                _ ->
                    wrcall({set_resp_header, ?CONTENT_ENCODING, ChosenEnc})
            end,
